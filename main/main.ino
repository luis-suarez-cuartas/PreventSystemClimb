...
const float tensionCuerdaArribaMax = 5;
const float tensionCaidaMax = 40;   
const float tensionReposoMax = 15;  

const float tensionCuerdaArribaMin = 0; 
const float tensionCaidaMin = 20;   
const float tensionReposoMin = 8;  
...
/*void createClimbedRouteSession(int trainingSessionId) {
  ...
  // Comprobar que el trainingSessionId no es 0 o un valor incorrecto
  Serial.print("Comprobando trainingSessionId: ");
  Serial.println(trainingSessionId);
  ...
  if (conn.connected()) {
    Serial.println("Conexión a la base de datos establecida correctamente.");
  } else {
  ...
  MySQL_Query query_mem_select = MySQL_Query(&conn);
  ...
  Serial.println("Ejecutando consulta para obtener el último ID...");
  if (query_mem_select.execute(SELECT_SQL.c_str())) {
    Serial.println("Consulta SELECT ejecutada correctamente.");
  } else {
  ...
  row_values *row = query_mem_select.get_next_row();
  if (row != NULL) {
    rutaActivaId = atoi(row->values[0]);  // Convertir el resultado a entero
  ...
  // Verificar si el valor de rutaActivaId es válido
  if (rutaActivaId > 0) {
    Serial.print("Nuevo ID de sesión almacenado: ");
    Serial.println(rutaActivaId);
  } else {
    Serial.println("Error: rutaActivaId no es válido.");
  }
}
...
void testSimpleSelect() {
  // Consulta muy simple para obtener el primer id de la tabla training_training
  String SELECT_SQL = "SELECT id FROM training_training LIMIT 1;";
  
  MySQL_Query query_mem = MySQL_Query(&conn);
  
  if (conn.connected()) {
    // Mostrar la consulta en la consola para ver qué se está ejecutando
    MYSQL_DISPLAY(SELECT_SQL);
    ...
    if (query_mem.execute(SELECT_SQL.c_str())) {
      ...
      if (cols) {
        ...
        if (row != NULL) {
          Serial.print("ID obtenido de training_training: ");
          Serial.println(row->values[0]);  // Mostrar el ID obtenido
        } else {
          Serial.println("No se encontró ningún resultado.");
        }
      } else {
        Serial.println("Error leyendo columnas.");
      }
    } else {
      Serial.println("Error ejecutando la consulta SELECT.");
    }
  } else {
    Serial.println("Error: No hay conexión a la base de datos.");
  }
}
  ...
  switch (estado) {
    ...
    case 12:  //////////////////////// ESPERANDO QUE COJAN LA CUERDA
      ...
      if (trainingId != 33) {
        ...
        if (conn.connected()) {
          Serial.println(trainingId);
          createTrainingForUser(1);  // Crear la sesión de ruta escalada
          Serial.println(rutaActivaId);
          startTime = millis();
          estado = 18;
          registrar(18);
    ...
    case 24:  ///////////////////////////// 4/4
      ...
      if (alturaActual == alturas[3]) {
        updateSectionTime(4); 
        registrar(26);
        Serial.println("Ha alcanzado la altura máxima");
        estado = 26;  //Siguiente nivel
    ...
    case 26:  /////////////////////////////// Subida terminada
      Serial.println("Estado: 26 - Subida terminada");
      registrar(26);
      estado = 30;  //Siguiente nivel
      break;
...
bool tensionChange() {
  ...
  int OFFSET_TENSION = 3;
  static float tensionAnterior;
  static unsigned long timerSensorChange;
  if (millis() > timerSensorChange + 10) {
    float tensionActual = medirTension();
...
int registrar(int eventoRecibido) {
  ...
  */
  Serial.print("Registrar evento ");
  Serial.print(eventoRecibido);
  Serial.println("");
...
void enviarDatos() {
  if (WiFi.status() == WL_CONNECTED) { // Asegurarse de que estamos conectados a WiFi
    ...
    String url = String(serverTS) + "/update?api_key=" + apiKey + "&field1=" + String(temperatura);
